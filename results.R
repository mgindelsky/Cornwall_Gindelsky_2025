...
no_penalty_gini <- c('taxcred', 'wages', 'prop', 'assets','covid_years')
no_penalty_q <- c('taxcred', 'wages', 'prop', 'assets','covid_years', 'gini_pred')
...
gini_x <- data %>%
  ...
  mutate(covid_years = ifelse(year==2020|year==2021,1,0))
...
q_x <- data %>%
  mutate(l1_q1 = lag(is_q1, 1),
         ...
         l2_q4 = lag(is_q4, 2),
         l2_q5 = lag(is_q5, 2)) %>%
  dplyr::select(-contains('is_q'), -gini) %>%
  mutate(indicator_2019 = ifelse(year == 2019, 1, 0)) %>%
mutate(covid_years = ifelse(year==2020|year==2021,1,0))
...
for (i in 1:4) {
  ...
  for (j in seq_along(alpha_check)) {
    ...
    predictions2 <- predict(check, newx = tempq, 
                            s = s_use) %>% 
      matrix(., nrow = 25, ncol = 5)
    
    RMSE[j] <- sum((predictions2[1:(20+i)] - targets %>% 
                      filter(year %in% 2000:(2019 + i)) %>%
...
q5_dat <- data.frame(year = data$year,
                     true = data$is_q5,
                     predictions_e20 = resultsq[[1]]$predictions[,5],
                     predictions_e21 = resultsq[[2]]$predictions[,5],
                     predictions_e22 = resultsq[[3]]$predictions[,5],
                     predictions_e23 = resultsq[[4]]$predictions[,5]) %>%
  mutate(across(-year, 
                .fns = ~round(.x, 1))) %>%
  mutate(predictions_e20 = case_when(
    year %in% 2021:2024 ~ NA,
    TRUE ~ predictions_e20
  ),
  predictions_e21 = case_when(
    year %in% 2022:2024 ~ NA,
    TRUE ~ predictions_e21
  ),
  predictions_e22 = case_when(
    year %in% 2024 ~ NA,
    TRUE ~ predictions_e22))
